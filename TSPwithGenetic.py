...
#SC Stop condition
#M is Mutation
#LSP is List of the shortest paths found
import openpyxl
import numpy as np
import random
    ...
    while len(CH) <instance :
        ...
        if N == [] and U != [] :
            ...
            for i in range(len(U)) :
                if sheet.cell(row=CH[-1] + 1, column=U[i] + 1).value < minimum_1:
                    minimum_1 = sheet.cell(row=CH[-1] + 1, column=U[i] + 1).value
                    asdf = U[i]
            ...
            for i in range(len(N)) :
                if minimum_1 > sheet.cell(row=CH[-1] + 1, column=N[i] + 1).value:
                    minimum_1 = sheet.cell(row=CH[-1] + 1, column=N[i] + 1).value
                    asdf = N[i]
